...
resource "google_compute_instance" "instance" {
  ...
  metadata = {
    ssh-keys = "${var.linux_user}:${tls_private_key.tls.public_key_openssh}"
  }
...
resource "null_resource" "ansible_playbook" {
  ...
  provisioner "local-exec" {
    ...
    command = var.is_public && length(var.ansible_skip_tags) > 0 ? "sleep 30 && ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${google_compute_instance.instance.network_interface.0.access_config.0.nat_ip}, -u ${var.linux_user} --private-key=id_rsa.pem playbook.yml  --extra-vars '@ansible_vars.json' --tags ${join(",", var.ansible_tags)} --skip-tags ${join(",", var.ansible_skip_tags)}": (
                var.is_public && length(var.ansible_skip_tags) <= 0 ? "sleep 30 && ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${google_compute_instance.instance.network_interface.0.access_config.0.nat_ip}, -u ${var.linux_user} --private-key=id_rsa.pem playbook.yml  --extra-vars '@ansible_vars.json' --tags ${join(",", var.ansible_tags)}": (
                  !var.is_public && length(var.ansible_skip_tags) > 0 ? "sleep 30 && ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${google_compute_instance.instance.network_interface.0.network_ip}, -u ${var.linux_user} --private-key=id_rsa.pem playbook.yml  --extra-vars '@ansible_vars.json' --tags ${join(",", var.ansible_tags)} --skip-tags ${join(",", var.ansible_skip_tags)}":"sleep 30 && ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i ${google_compute_instance.instance.network_interface.0.network_ip}, -u ${var.linux_user} --private-key=id_rsa.pem playbook.yml  --extra-vars '@ansible_vars.json' --tags ${join(",", var.ansible_tags)}"
                )
              )
  }
